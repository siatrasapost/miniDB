...
class Table:
    ...
    def _full_outer(self, table_right: Table, condition):
        '''
        Full Outer Join table (left) with a supplied table (right) where condition is met.
        '''
        # get columns and operator
        column_name_left, operator, column_name_right = self._parse_condition(condition, join=True)
        ...
        # define the new tables name, its column names and types
        join_table_name = f'{self._name}_full-outer-join_{table_right._name}'
        join_table_colnames = left_names+right_names
        join_table_coltypes = self.column_types+table_right.column_types
        join_table = Table(name=join_table_name, column_names=join_table_colnames, column_types= join_table_coltypes)
    ...
    def _left_outer(self, table_right: Table, condition):
        '''
        ...
        # define the new tables name, its column names and types
        join_table_name = f'{self._name}_left-outer-join_{table_right._name}'
        join_table_colnames = left_names+right_names
        join_table_coltypes = self.column_types+table_right.column_types
        join_table = Table(name=join_table_name, column_names=join_table_colnames, column_types= join_table_coltypes)
        ...
        for row_left in self.data:
            ...
            for row_right in table_right.data:
                right_value = row_right[column_index_right]
                no_of_ops+=1
                if get_op('==', left_value, right_value): #EQ_OP
                    temp.append([True, row_left+row_right])
                else:
                    temp.append([False, row_left])

            for i in range (len(temp)):
                if temp[i][0] is False:
                    counter+=1           
            if counter==len(table_right.data):
                for i in range (table_right._no_of_columns):
                    null_list.append(0)
                join_table._insert(row_left+null_list)
            else:
                for i in range (len(temp)):
                    if temp[i][0] is True:
                        join_table._insert(temp[i][1])
    ...
    def _right_outer(self, table_right: Table, condition):
        '''
        Right Outer Join table (left) with a supplied table (right) where condition is met.
        '''
        # get columns and operator
        column_name_left, operator, column_name_right = self._parse_condition(condition, join=True)
        ...
        # define the new tables name, its column names and types
        join_table_name = f'{self._name}_right-outer-join_{table_right._name}'
        join_table_colnames = left_names+right_names
        join_table_coltypes = self.column_types+table_right.column_types
        join_table = Table(name=join_table_name, column_names=join_table_colnames, column_types= join_table_coltypes)
        ...
        for row_right in table_right.data:
            ...
            if counter==len(self.data):
                for i in range (self._no_of_columns):
                    null_list.append(0)
                join_table._insert(null_list+row_right)
            else:
                for i in range (len(temp)):
                    if temp[i][0] is True:
                        join_table._insert(temp[i][1])
